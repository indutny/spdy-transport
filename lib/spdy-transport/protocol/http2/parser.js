...
Parser.prototype.onFrameHead = function onFrameHead(buffer, callback) {
  ...
  if (this.isServer && header.id != 0) {
    if (header.id % 2 == 0) {
      return callback(this.error(constants.error.PROTOCOL_ERROR,
                                 'Server received even stream-id'));
    }
    if (header.id < this.lastStream) {
      return callback(this.error(constants.error.PROTOCOL_ERROR,
                                 'Server received stream-id less than previous'));
    }
    this.lastStream = header.id;
  }
...
Parser.prototype.onDataFrame = function onDataFrame(header, body, callback) {
  ...
  if (this._lastHeaderBlock === null && this.pendingHeader === null) {
    return callback(this.error(constants.error.PROTOCOL_ERROR,
...
Parser.prototype.onRSTFrame = function onRSTFrame(header, body, callback) {
  ...
  if (this._lastHeaderBlock === null && this.pendingHeader === null) {
    return callback(this.error(constants.error.PROTOCOL_ERROR,
                               'Received RST_STREAM on idle stream'));
  }
...
Parser.prototype.onSettingsFrame = function onSettingsFrame(header,
  ...
  if (constants.settingsIndex[constants.settings_server.SETTINGS_ENABLE_PUSH] in settings) {
    if ((settings[constants.settingsIndex[constants.settings_server.SETTINGS_ENABLE_PUSH]] !== 0)
     && (settings[constants.settingsIndex[constants.settings_server.SETTINGS_ENABLE_PUSH]] !== 1)) {
      return callback(this.error(constants.error.PROTOCOL_ERROR,
                                 'SETTINGS_ENABLE_PUSH must be 0 or 1'));
     }
  }
  if (constants.settingsIndex[constants.settings_server.SETTINGS_INITIAL_WINDOW_SIZE] in settings) {
    if ((settings[constants.settingsIndex[constants.settings_server.SETTINGS_INITIAL_WINDOW_SIZE]] > constants.MAX_INITIAL_WINDOW_SIZE)
     || (settings[constants.settingsIndex[constants.settings_server.SETTINGS_INITIAL_WINDOW_SIZE]] < 0)) {
      return callback(this.error(constants.error.FLOW_CONTROL_ERROR,
                                 'SETTINGS_INITIAL_WINDOW_SIZE is OOB'));
     }
  }
  if (constants.settingsIndex[constants.settings_server.SETTINGS_MAX_FRAME_SIZE] in settings) {
    if ((settings[constants.settingsIndex[constants.settings_server.SETTINGS_MAX_FRAME_SIZE]] > constants.ABSOLUTE_MAX_FRAME_SIZE)
     || (settings[constants.settingsIndex[constants.settings_server.SETTINGS_MAX_FRAME_SIZE]] < constants.INITIAL_MAX_FRAME_SIZE)) {
      return callback(this.error(constants.error.PROTOCOL_ERROR,
...
Parser.prototype.onPushPromiseFrame = function onPushPromiseFrame(header,
  ...
  if (this.isServer) {
    return callback(this.error(constants.error.PROTOCOL_ERROR,
                               'Server received a PUSH_PROMISE'));
  }
  if (header.id === 0) {
    return callback(this.error(constants.error.PROTOCOL_ERROR,
...
Parser.prototype.onWindowUpdateFrame = function onWindowUpdateFrame(header,
  ...
  if (this._lastHeaderBlock === null && this.pendingHeader === null) {
    return callback(this.error(constants.error.PROTOCOL_ERROR,
                               'Received WINDOW_UPDATE on idle stream'));
  }
